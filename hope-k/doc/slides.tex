...
\begin{frame}
 ...
 Poiché le istanze del problema sono delle molecole, ai fini della classificazione è necessario definire una misura di somiglianza fra di esse. Si noti come senza l'ausilio dei kernel le operazioni tendano di gran lunga a complicarsi.
...
\begin{frame}
 ...
 \begin{itemize}
  ...
  \item Ricerca con algoritmo DFS a partire da ciascun nodo, profondità variabile\\ (particolarità dei grafi con cicli)
...
\begin{frame}
 ...
 \begin{columns}[c]
  ...
  \column{0.50\textwidth}
   ...
   \vbox{\vskip10pt}
   \begin{block}{Risultati}
    \# \textit{path} mol. 1: \textbf{52}\\ \# \textit{path} mol. 2: \textbf{38}\\ \# \textit{path} comuni: \textbf{27}\\ Somma molteplicità minime: \textbf{33}\\ Somma molteplicità massime: \textbf{111}\\ Valore del Kernel: \textbf{0.2973}\\
   \end{block}
 \end{columns}
\end{frame}
...
\begin{frame}
 ...
 \begin{block}<2->{Tecniche: 10 Fold Cross Validation e 80-20 Cross Validation}
  \begin{itemize}
   \item<3-> \alert<3,3>{\textbf{10 Fold Cross Validation:}} la tecnica è stata sfruttata per l'analisi ed il confronto dei due kernel generati, così da individuare sia quale dei due presenta un comportamento migliore, quanto quale risulta essere la profondità ottimale di ricerca
   \item<4->
